...
import no.ntnu.progark.towerdefense.io.ImageHelper;
import no.ntnu.progark.towerdefense.io.MapReader;
import no.ntnu.progark.towerdefense.tiled.core.BasicTileCutter;
import no.ntnu.progark.towerdefense.tiled.util.Base64;
...
//                                "' on <" + node.getNodeName() + "> tag");
    ...
    private Bitmap unmarshalImage(Node t) throws IOException
    {
        Bitmap img = null;
        ...
        if (source != null) {
                throw new IOException("Not supporting external TSX files");
...
//                    + e.getLocalizedMessage());
    ...
    private TileSet unmarshalTileset(Node t) throws Exception {
        ...
        //file, this attribute refers to that file.
        String source = getAttributeValue(t, "source");
...
//                        logger.warn("Ignoring illegal image element after tileset image.");
                    ...
                    if (imgSource != null && id == null) {
                        ...
                        String tileBitMapName = imgSource;
                        tileBitMapName = tileBitMapName.substring(0, 
                                        tileBitMapName.lastIndexOf('.'));
...
//                        logger.info("Importing " + sourcePath + "...");
                        ...
                        if (transStr != null) {
                            set.setTransparentColor(Color.parseColor(transStr));
                        }
                        set.importTileBitmap(resources, tileBitMapName, new BasicTileCutter(
                                tileWidth, tileHeight, tileSpacing, tileMargin));
                    } else {
                        Bitmap image = unmarshalImage(child);
    ...
    private MapObject readMapObject(Node t) throws Exception {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if ("image".equalsIgnoreCase(child.getNodeName())) {
                ...
                if (source != null) {
                        throw new Exception("No support for map objects containing images!");
                }
                break;
            }
        }
        ...
        throws Exception
    {
        ...
        try {
            if (isAnimated) {
                throw new Exception("No support for animated tiles!");
...
//            logger.error("failed creating tile: "+e.getLocalizedMessage());
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if ("image".equalsIgnoreCase(child.getNodeName())) {
                ...
                Bitmap img = unmarshalImage(child);
                if (id < 0) {
                    id = set.addImage(img);
                }
                tile.setImage(id);
...
//                        logger.warn("layer <data> tag enclosed no data. (empty data tag)");
    ...
    @Override
        public Map readMap(Resources res, int resourceId) throws Exception {
                this.resources = res;
